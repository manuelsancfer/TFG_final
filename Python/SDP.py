            ...
            if key != 'media':
                ...
                for idx in range(0, len(sdp_dict['media'])):
                    ...
                    self.media_list.append(
                        SDPMedia(media, port, protocol, numports, fmt, connection, attributes, bandwidth))
...
class SDPMedia:
    ...
    def __init__(self, media, port, transport, port_count=1, formats=None, connection=None, attributes=None,
                 bandwidth_info=None):
        self.media = media
        self.port = port
...
def _parseline(line):
    ...
    if type == "v":
        ...
        media, port, numports, protocol, fmt = re.match(
            "^(audio|video|text|application|message) +(\d+)(?:[/](\d+))? +([^ ]+) +(.+)$", value).groups()
        port = int(port)
        if numports is None:
            numports = 1
        else:
